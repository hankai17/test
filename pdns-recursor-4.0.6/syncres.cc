...
extern icscheck* pIcsCheck;
extern bool icscheck_button;

int SyncRes::beginResolve(const DNSName &qname, const QType &qtype, uint16_t qclass, vector<DNSRecord>&ret, int groupid)
{
  if (icscheck_button == true) {
  ...
  rr.d_ttl = 120;
  if (qtype.getCode() == QType::A) {
     if( (*((*pconf).str_servers[groupid-1])).ipv4_server.size() > 0 ) {
         if  ( (int)(pIcsCheck->ics_ploy_list)[groupid-1]->alive_ipv4_ics_count > 0 ) {
             for(int i=0; i<(int)(pIcsCheck->ics_ploy_list)[groupid-1]->alive_ipv4_ics_count;i++) {
                rr.d_content=shared_ptr<DNSRecordContent>(DNSRecordContent::mastermake(QType::A, 1,(pIcsCheck->ics_ploy_list)[groupid-1]->ics_ipv4_array[i]->ip->data));
                 ret.push_back(rr);
             }
         } else {
             d_wasOutOfBand=true;
         ...
         if  ( (int)(pIcsCheck->ics_ploy_list)[groupid-1]->alive_ipv6_ics_count == 0 ) {
             d_wasOutOfBand=true;
             goto new_start;
         }
     }
 } else {
     if( (*((*pconf).str_servers[groupid-1])).ipv6_server.size() > 0 ) {
         if  ( (int)(pIcsCheck->ics_ploy_list)[groupid-1]->alive_ipv6_ics_count > 0 ) {
             for(int i=0; i<(int)(pIcsCheck->ics_ploy_list)[groupid-1]->alive_ipv6_ics_count;i++) {
                rr.d_content=shared_ptr<DNSRecordContent>(DNSRecordContent::mastermake(QType::AAAA, 1,(pIcsCheck->ics_ploy_list)[groupid-1]->ics_ipv6_array[i]->ip->data));
                 ret.push_back(rr);
             }
         } else {
		 /*
  ...
  if( (qtype.getCode() == QType::A || qtype.getCode() == QType::AAAA) && groupid > 0 )
 {
  ...
  if (qtype.getCode() == QType::A) {
     if( (*((*pconf).str_servers[groupid-1])).ipv4_server.size() > 0 ) {
        for(int i = 0; i < (int)(*((*pconf).str_servers[groupid - 1])).ipv4_server.size(); i++) {
          rr.d_content=shared_ptr<DNSRecordContent>(DNSRecordContent::mastermake(QType::A, 1, (*((*pconf).str_servers[groupid - 1])).ipv4_server[i]->data));
          ret.push_back(rr);
        }
     } else {
        d_wasOutOfBand=true;
     ...
     if( (*((*pconf).str_servers[groupid-1])).ipv6_server.size() > 0 ) {
        for(int i = 0; i < (int)(*((*pconf).str_servers[groupid - 1])).ipv6_server.size(); i++) {
          rr.d_content=shared_ptr<DNSRecordContent>(DNSRecordContent::mastermake(QType::AAAA, 1, (*((*pconf).str_servers[groupid - 1])).ipv6_server[i]->data));
          ret.push_back(rr);
        }
     }
 }
...
int SyncRes::beginResolve(const DNSName &qname, const QType &qtype, uint16_t qclass, vector<DNSRecord>&ret)
{
  if( (qtype.getCode() == QType::A || qtype.getCode() == QType::AAAA) && (*pconf).in_white_flag == 1 && (*pconf).in_black_flag == 0 )
 {
  ret.clear();
  DNSRecord rr;
  rr.d_name = qname;
  ...
  rr.d_ttl = 120;
  if (qtype.getCode() == QType::A) { 
     if( (*((*pconf).str_servers[(*pconf).group-1])).ipv4_server.size() > 0 ) { 
         if  ( (int)(pIcsCheck->ics_ploy_list)[(*pconf).group-1]->alive_ipv4_ics_count > 0 ) {
             for(int i=0; i<(int)(pIcsCheck->ics_ploy_list)[(*pconf).group-1]->alive_ipv4_ics_count;i++) {             
                rr.d_content=shared_ptr<DNSRecordContent>(DNSRecordContent::mastermake(QType::A, 1,(pIcsCheck->ics_ploy_list)[(*pconf).group-1]->ics_ipv4_array[i]->ip->data));
                 ret.push_back(rr);
             } 
         } else {
             d_wasOutOfBand=true;
             (*pconf).in_white_flag = 0;
             goto new_start;
         }
     } else {
         if  ( (int)(pIcsCheck->ics_ploy_list)[(*pconf).group-1]->alive_ipv6_ics_count == 0 ) {
             d_wasOutOfBand=true;
             (*pconf).in_white_flag = 0;
             goto new_start;
         }
     ...
     if( (*((*pconf).str_servers[(*pconf).group-1])).ipv6_server.size() > 0 ) {
         if  ( (int)(pIcsCheck->ics_ploy_list)[(*pconf).group-1]->alive_ipv6_ics_count > 0 ) {
             for(int i=0; i<(int)(pIcsCheck->ics_ploy_list)[(*pconf).group-1]->alive_ipv6_ics_count;i++) {         
                rr.d_content=shared_ptr<DNSRecordContent>(DNSRecordContent::mastermake(QType::AAAA, 1,(pIcsCheck->ics_ploy_list)[(*pconf).group-1]->ics_ipv6_array[i]->ip->data));
                 ret.push_back(rr);
             } 
         } else {
             d_wasOutOfBand=true;
