...
int dns_conf::parse_white_conf()
{
    ...
    std::ostringstream _strline;
    int new_flag = 0;
    int group_flag = 1;//flag=0Ê± ÕšÒ»Â¸
    ip_obj* str_p = NULL;
    int group1=0;//ÂµÚ¼Â¸Ø©

    int servers_flag=0;//serversÂ¿ÂªÊ¼
    int domains_flag=0;//domainsÂ¿ÂªÊ¼
    ...
    while (c_white_filestream.get(ch)) {
        ...
        if(ch == '}')
            block_num--;
        if(block_num == 0)
        {   //ÂµÂ±Ò»Â¸í¬¢í¹½â‹¸ ×ƒÂ±ë–¾Î»,Í¬Ê±È¥Â¿ÔŸstrline
            //Â²Â»ÅœÈ¥Â¿
            group_flag=1;
        }
        ...
        if(ch == '=')//Ò»Â¸í¬‚í²—é¿ªÊ¼ , Â¼Ú‰é¡Ø©oÂ²Â»Õ™Ç¶Í— 
        {
        ...
        if(servers_flag == 1)//Â½ã¶serversĞ¡Ø©
        {
            if (new_flag == 0)//Ö»Â½í¢•í²»Â´
            {
		...
		str_new_p->group_id = group1;
            ...
            if (ch == '"')
            {
                ref_num++;
                if(ref_num == 1)
                    _strline.str("");//ÂµÛ’Â»Â¸í¡‚í±”í«†í²¼Â´È¥Â¿ÔŸstrline
                continue;
            }
            if (ref_num == 2)
            {
            ...
            if(ch == '}')    //Ò»Â¸í¬‚í²—é¶„Â½â‹¸Â±ë–¾.ÂµÂ«Ë‡É§Â¹í „í·¶Í— Å‡Â¾ÍµÄŞ¸Å
            {
                servers_flag = 0;
                _strline.str("");
            }
        }
        if( domains_flag == 1)//Â½ã¶Ô²Ä»Ğ¡Ø©
        {
            //Ö¢Ö®Ç°ref_numÂ±Ùìª0
            if (ch == '"')
            {
                ref_num++;
                if(ref_num == 1)
...
int dns_conf::check(char* str_domain, int len, int* gid) {
    ...
    if ( ( iter = str_domain_hash.find(str_domain)) != str_domain_hash.end() ) { //Â¾Â«×¼Ô²Ä»ÆÂ¶
        if (iter->second->group_id == -1) { //ÂºÚ¾Â«×¼Ô²Ä»ÂµÃ©pØ©Îª-1
            *gid = -99;
            return 1;
        } else if (iter->second->group_id > 0 && iter->second->group_id <= (int)str_servers.size()) {
	        *gid = iter->second->group_id; //group_idÂ´Ñ±Â¿ÂªÊ¼
        }
    }
    if (head_flag == 1) {
