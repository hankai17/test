  ...
  *now=dt.getTimeval();
  ...
  try {
    ...
    if(EDNS0Level > 0 && getEDNSOpts(mdp, &edo)) {
	  ...
	  std::cout<<"resp have ednsopt !!"<<std::endl;
      if(weWantEDNSSubnet) {
        for(const auto& opt : edo.d_options) {
          if(opt.first==8) {
            ...
            if(getEDNSSubnetOptsFromString(opt.second, &reso)) {
             ...
             std::cout<<"EDNS Subnet response: "<<reso.source.toString()<<", scope: "<<reso.scope.toString()<<", family = "<<reso.scope.getNetwork().sin4.sin_family<<endl;
              ...
              if(reso.scope.getBits())
				std::cout<<"have srcmask !!"<<std::endl;
                srcmask = reso.scope;
            }
          }
        }
