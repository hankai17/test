...
dns_conf* pconf = nullptr;
icscheck* pIcsCheck = nullptr;
bool icscheck_button;
...
void startDoResolve(void *p)
{
  ...
  try {
    ...
    if(!t_pdl->get() || !(*t_pdl)->preresolve(dc->d_remote, dc->d_local, dc->d_mdp.d_qname, QType(dc->d_mdp.d_qtype), dc->d_tcp, ret, dc->d_ednsOpts.empty() ? 0 : &dc->d_ednsOpts, dc->d_tag, &appliedPolicy, &dc->d_policyTags, res, &variableAnswer, &wantsRPZ)) {
      ...
      try {
        ...
        res = sr.beginResolve(dc->d_mdp.d_qname, QType(dc->d_mdp.d_qtype), dc->d_mdp.d_qclass, ret, dc->groupid);
  ...
  haveAnswer:;
    ...
    if(auto rec = std::dynamic_pointer_cast<ARecordContent>(i->d_content)) {
          //std::cout<<"i->d_content="<< (rec->getCA(53)).toString()<<std::endl;
          last_ip = (rec->getCA(53)).toString().c_str();
    }
  ...
  sendit:;
	...
	addCMsgSrcAddr(&msgh, cbuf, &dc->d_local, 0);
      ...
      if (last_ip != nullptr) {
        L<<Logger::Notice<<"query for "<<dc->d_mdp.d_qname.toLogString()<<" from "<<dc->getRemote()<<" type="<<dc->d_mdp.d_qtype<<" "<<"record="<<last_ip<<std::endl;
      } else {
        L<<Logger::Notice<<"query for "<<dc->d_mdp.d_qname.toLogString()<<" from "<<dc->getRemote()<<" type="<<dc->d_mdp.d_qtype<<" "<<std::endl;
      }


      if(!SyncRes::s_nopacketcache && !variableAnswer && !sr.wasVariable() ) {
		std::cout<<"insert packet cache "<<std::endl;
...
string* doProcessUDPQuestion(const std::string& question, const ComboAddress& fromaddr, const ComboAddress& destaddr, struct timeval tv, int fd)
{
  ...
  try {
    ...
    */
    ...
    int gid = -99;
    (*pconf).check((char*)toLower(dc->d_mdp.d_qname.toLogString()).c_str(),(int)dc->d_mdp.d_qname.toLogString().length(), &gid);
    //(*pconf).check1((char*)toLower(dc->d_mdp.d_qname.toLogString()).c_str(),(int)dc->d_mdp.d_qname.toLogString().length(), &gid);
    if (gid <= (int)pconf->str_servers.size() && gid > 0) {
        dc->groupid = gid;
    } else {
        dc->groupid = -99;
    }
 ...
 if(gid < 0) {
	...
	addCMsgSrcAddr(&msgh, cbuf, &destaddr, 0);
      ...
      L<<Logger::Notice<<"hit query for "<<dc->d_mdp.d_qname.toLogString()<<" from "<<dc->getRemote()<<" type="<<dc->d_mdp.d_qtype<<" "<<std::endl;
...
int serviceMain(int argc, char*argv[])
{
  ...
  if(icscheck_button == true) {
    if(pIcsCheck == nullptr) {
        pIcsCheck = new icscheck();
        std::cout << "---------------------------->g_numThreads is 1, icscheck started\n";
    }
    sleep(1);
    recursorThread(0);
    }
...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    ::arg().set("ipv6", "If we turn on ipv6-function")="off";  
    ::arg().set("servers_probe", "If we turn on servers_probe-function")="on"; //默认开启探测功能 
  ...
  if(pconf == nullptr)
  {
     pconf = new dns_conf(::arg()["config-dir"]);
  }
  (*pconf).open_conf_file();
  if( (*pconf).parse_black_conf() != 0)
  {
    std::cout<<"parse_black_conf err !"<<std::endl;
    L<<Logger::Error<<"Exception: "<<"parse_black_conf err"<<endl;
    std::exit(1);
  }
  if( (*pconf).parse_white_conf() != 0)
  {
    std::cout<<"parse_white_conf err !"<<std::endl;
    L<<Logger::Error<<"Exception: "<<"parse_white_conf err"<<endl;
    std::exit(1);
  }
  (*pconf).close_conf_file();

  if((*pconf).ipv6_button_check(::arg()["ipv6"]) != 0) {  
    std::exit(1);
  }
  
  if(::arg()["servers_probe"].find("off", 0) != ::arg()["servers_probe"].npos) {
    icscheck_button = false; //关闭 
  } else {
    icscheck_button = true; //关闭 
  }
