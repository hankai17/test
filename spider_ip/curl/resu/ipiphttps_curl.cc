...
char curl_ics(std::string &chip)
{
	...
	memcpy(User_Agent,"Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1)", 
               strlen("User-Agent: Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1"));

    //memcpy(url, "https://ip.rtbasia.com/?ipstr=123.120.175.12", strlen("https://ip.rtbasia.com/?ipstr=123.120.175.12")); //cache-control no-cache
    memcpy(url, "https://ip.rtbasia.com/?ipstr=113.120.175.1", strlen("https://ip.rtbasia.com/?ipstr=113.120.175.1")); //cache-control no-cache
    curl_global_init(CURL_GLOBAL_DEFAULT); //https
