...
class tb_spider_ip: public spider_ip
{
    ...
    public:
        ...
        /*
        void change_2_utf8(std::string str)
        {
            ...
            while(foo != NULL)
            {
                ...
                std::string hex_num = "0x" + (std::string s = { foo };
                std::cout<< "--->"<<hex_num <<std::endl;
                //std::cout<<(unsigned long)atol(foo)<<std::endl;
                foo = strtok(NULL,"\\u");
                //enc_unicode_to_utf8_one(unsigned long unic, unsigned char *pOutput,int outSize)  
            }
        }
        */
...
void test()
{
    DoBussiness(new tb_spider_ip("1.0.8.0","1.0.15.255"));
    //sleep(1);
}
