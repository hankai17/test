...
int main()
{
    ...
    char str_ip[16] = {0};
    int fd;
    fd = open("ip.txt", O_RDWR|O_TRUNC|O_CREAT,0664);
    for (int m = 0; m < 100; m++)
        for (int n = 0; n < 1; n++)
            //循环IP段开始爬去数据
            for (int i = ip[m][0]; i <= ip[m][4]; i++)  //1次
                for (int ii = ip[m][1]; ii <= ip[m][5]; ii++) //4次
                    for (int iii =ip[m][2]; iii<= ip[m][6]; iii++) //256次
                        for (int iiii = ip[m][3]; iiii <= ip[m][7]; iiii++) //256次 
                        {
                            //std::cout<<i<<" "<<ii<<" "<<iii<<" "<<iiii<<std::endl;
                            sprintf( str_ip,"%d.%d.%d.%d\n",i,ii,iii,iiii);
                            //std::cout<<str_ip<<std::endl;
                            write(fd,str_ip,strlen(str_ip));
                            memset(str_ip,0,16);
