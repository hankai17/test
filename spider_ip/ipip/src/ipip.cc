...
int ipip_spider_ip::write_data(char* page_data,size_t size, size_t nmemb,std::string *data)
{
    if(data == NULL)
        return 0;
...
ipip_spider_ip::ipip_spider_ip(std::string ip_b, std::string ip_e)
{
    step = 0;
    ip_back = 0;
    current_result = "";
    ...
    memcpy(User_Agent,"User-Agent: Mozilla/5.0 (Windows NT 6.1; \
        ...
        WOW64; rv:52.0) Gecko/20100101 Firefox/52.0"));
    memcpy(url, "http://freeapi.ipip.net/", strlen("http://freeapi.ipip.net/"));
    char file_name[64] = {0};
    sprintf(file_name,"%s~~%s_ipip.txt",(char*)ip_b.data(),(char*)ip_e.data());
    std::cout<<"file_name="<<file_name<<std::endl;
    fd = open(file_name, O_RDWR|O_TRUNC|O_CREAT, 0664);
}

ipip_spider_ip::~ipip_spider_ip()
{
    //close file
    std::cout<<"~ipip_spider_ip()"<<std::endl; 
}

std::string ipip_spider_ip::confirm_url(std::string ip)
{
    if(once_flag == 0)
    {
    ...
    while(1)
    {
        ...
        if(current_url == "NULL")
        {
            if(write_last_ip != ip_end)
            {
                ...
                Log(__FILE__, __LINE__,Level[2],0,"%s",(char*)(ip_end.data()));
            }
            break;
        }
...
//确保传入的参数第一个字符非空格
std::string ipip_spider_ip::revise_result(std::string& str)
{
    if(str.data() == nullptr)
    {
        return "NULL";
...
void ipip_spider_ip::parse_data()
{
    if(buffer.size() == 0)
    {
...
void ipip_spider_ip::insert_db()
{
    //std::cout << "insert_db" << std::endl;
}

int ipip_spider_ip::code_convert(char *from_charset,char *to_charset,char *inbuf,size_t inlen,char *outbuf,size_t outlen)  
{  
    iconv_t cd;  
    char **pin = &inbuf;  
    char **pout = &outbuf;  
...
int ipip_spider_ip::g2u(char *inbuf,size_t inlen,char *outbuf,size_t outlen)  
{  
