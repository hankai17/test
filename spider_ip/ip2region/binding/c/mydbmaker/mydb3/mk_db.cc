...
void parse_buf(char* buffer, std::ofstream& f1) //must use ref
{
	if(buffer == nullptr)
		return;
...
int main(int argc,char*argv[])
{
	...
	char buffer[256] = {0};
	std::ofstream f1("ip.db",std::ios::binary|std::ios::out|std::ios::trunc);  //must use write, other than << !
	...
	while(file.peek() != EOF)
	{
		...
		parse_buf(buffer,f1); //写入region  f1是对象
	...
	for(auto& b : block_l)
	{
		...
		f1.write((char*)&e,sizeof(e)); //写入最小存储单元
	...
	{
		f1.write((char*)&h,sizeof(h)); //写入4k头 (暂时用不到)
	}
