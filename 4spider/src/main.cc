...
void test()
{
    auto f1 = std::bind(DoBussiness,std::placeholders::_1);
    vec_fun_ip138.push_back(f1);
    vec_fun_ip138[0](new ip138_spider_ip(ip_b[0],ip_e[0]));
...
void ip138()
{
    ...
    for(int j = 0; j < 100; j++)
    {
       std::thread t1(vec_fun_ip138[j](ip_b[j],ip_e[j])); 
       t1.join();
    }
    return;
}
