...
#ifdef HAVE_PROTOBUF
__thread boost::uuids::random_generator* t_uuidGsafdenerator;
#endif
...
string* doProcessUDPQuestion(const std::string& question, const ComboAddress& fromaddr, const ComboAddress& destaddr, struct timeval tv, int fd)
{
  ...
  try {
    ...
    */
    ...
    if(needECS || (t_pdl->get() && (*t_pdl)->d_gettag)) {// not down in this
      try {
  ...
  dc->d_ednssubnet = ednssubnet;
#endif
  (*pconf).check((char*)dc->d_mdp.d_qname.toLogString().c_str(),(int)dc->d_mdp.d_qname.toLogString().length());
...
{
  //std::cout<<"makeUDPServerSockets() fun"<<std::endl;
...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    ::arg().set("delegation-only","Which domains we only accept delegations from")="www.baidu.com";
