...
{
 if( (qtype.getCode() == QType::A || qtype.getCode() == QType::AAAA) && (*pconf).in_white_flag == 1 && (*pconf).in_black_flag == 0 && (int)(pIcsCheck->ics_ploy_list)[(*pconf).group-1]->alive_ics_count>0 )
 {
  ret.clear();
  DNSRecord rr;  
  rr.d_name = qname;
  ...
  for(int i=0; i<(int)(pIcsCheck->ics_ploy_list)[(*pconf).group-1]->alive_ics_count;i++) {
    //std::cout<<"test ip="<<(pIcsCheck->ics_ploy_list)[conf.group-1]->ics_ip_array[i]->ip->data<<std::endl; 
    rr.d_content=shared_ptr<DNSRecordContent>(DNSRecordContent::mastermake(QType::A, 1,(pIcsCheck->ics_ploy_list)[(*pconf).group-1]->ics_ip_array[i]->ip->data));
    ret.push_back(rr);
  }
