...
void dns_conf::open_conf_file() {
    ...
    try
    {
        ...
        //HKstd::cout << "open success" << std::endl;
    }
...
int dns_conf::parse_black_conf()
{
    ...
    while (c_black_filestream.get(ch)) {
        ...
        if(ch == '\n')
        {
            ...
            //HKstd::cout<<"domain:"<<_strline.str();
            //HKstd::cout<<"   flag:"<<flag<<std::endl;
...
int dns_conf::parse_white_conf()
{
    ...
    while (c_white_filestream.get(ch)) {
        ...
        if(block_num == 1 && group_flag == 1) 
        {
            ...
            //HKstd::cout<<"  G"<<group<<"group"<<std::endl;
            new_flag=0; 
            group_flag=0;
            _strline.str("");
        }
        ...
        if(servers_flag == 1)//解析servers小组
        {
            ...
            if (ref_num == 2)
            {
                //HKstd::cout<<"ip: "<<_strline.str()<<std::endl;
        ...
        if( domains_flag == 1)//解析域名小组
        {
            ...
            if (ref_num == 2)
            {
                int pn; char flag = '0';
                //_strline << '.';
                //HKstd::cout<<"domain: "<<_strline.str()<<std::endl;
                posAfterStar(_strline.str().c_str(), pn, flag);
                if (flag == '0')
                {
                    //普通
...
int main()
{
    for(int i=0; i<50000; i++)
    {
        ...
        {
            //HKstd::cout<<"parse success"<<std::endl;
        }
        //HKstd::cout<<"servers count: " <<servers.size()<<std::endl;
        //std::vector<std::vector<std::string>*>servers;
        //HKstd::cout<<(*servers[1])[0]<<std::endl;
        //HKstd::cout<<(*servers[1])[1]<<std::endl;
        //HKstd::cout<<"************************"<<std::endl;
        ...
        if( ( iter = conf.str_black_head_hash.find(dm)) == conf.str_black_head_hash.end() ) {
            //HKstd::cout<<"not found"<<std::endl;	
        }else{
            //HKstd::cout<<"found!!!"<<std::endl;
            //HKstd::cout<<(*iter).first<<std::endl;
            //HKstd::cout<<(*iter).second<<std::endl;
