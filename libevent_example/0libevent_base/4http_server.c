...
//http://192.168.155.177/data?cmd=newFile&fromId=0&count=8
void httpd_handler(struct evhttp_request* req, void* arg) {
  ...
  //得到q所对应的value
  sprintf(tmp, "cmd=%s\n", evhttp_find_header(&params, "cmd"));
  strcat(output, tmp);
  //得到s所对应的value
  sprintf(tmp, "fromId=%s\n", evhttp_find_header(&params, "fromId"));
  strcat(output, tmp);
...
int main(int argc, char *argv[]) {
  ...
  evhttp_set_gencb(httpd, httpd_handler, NULL); //指定generic callback //也可以为特定的URI指定callback

#if 0
  evhttp_set_cb(httpd, "/", specific_handler, NULL);
  evhttp_set_cb(httpd, "/upload", upload_busi, NULL);
  evhttp_set_cb(httpd, "/data", data_busi, NULL):
	evhttp_set_cb(httpd, "/reg",  reg_buso, NULL);
#endif
