...
void MTasker<Key,Val>::makeThread(func_t* start, void* val) {
...
#ifdef SOLARIS                                 //参数个数
  makecontext (uc, (void (*)(...))threadWrapper, 4, this, start, d_maxtid, val); //修改上下文指向threadWrapper函数 threadWrapper就是start还有后面的参数
#else
...
bool MTasker<Key,Val>::schedule() {
  if(!d_runQueue.empty()) { //运行队列不空，取头"tid" 根据tid找到线程map容器相应的 "线程" 
	...
	std::cout<<"schedule run return true\n";
	return true;
  }
  if(!d_zombiesQueue.empty()) {
	...
	d_zombiesQueue.pop();
	std::cout<<"schedule zombies return true\n";
	return true;
  }
  if(!d_waiters.empty()) {
	time_t now=time(0);
	for(typename waiters_t::const_iterator i=d_waiters.begin();i!=d_waiters.end();++i) {
	  ...
	  std::cout<<"schedule 没有超时\n";
	}
  }
  std::cout<<"schedule return false...\n";
  return false;
}
...
template<class Key, class Val>
void MTasker<Key,Val>::threadWrapper(MTasker* self, func_t* tf, int tid, void* val) {
  (*tf)(val);
