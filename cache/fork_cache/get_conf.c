...
{
    FILE *fp = NULL;
    fp = fopen(file, "r");  //cache.conf
    if(fp == NULL)
    perror("open file failed!:");

          if( get_message(fp,conf->port) == -1)
          {
            printf("no found the port!\n");
             fclose(fp);
            return -1;
          }
          ...
          {
            printf("no found the dev!\n");
            fclose(fp);
            return -1;
          }

          if(get_message(fp,conf->path_white) == -1)  //white.txt
          {
            printf("no found the path_white\n");
            fclose(fp);
            return -1;
          }
          ...
          if(get_message(fp,conf->path_server_ip) == -1) //cache_ip.server
          {
            printf("no found the server_ip\n");
            fclose(fp);
            return -1;
          }
          if(get_message(fp,conf->path_second_filter) == -1) //second_filter.server
          {
            printf("no found the second filter!\n");
            fclose(fp);
            return -1;
          }
          if(get_message(fp,conf->path_src_ip_white) == -1) //src_ip_white.txt
          {
            printf("no found the path_src_ip_white !\n");
            fclose(fp);
            return -1;
          }
          if(get_message(fp,conf->path_src_ip_black) == -1)
          {
            printf("no found the path_src_ip_black!\n");
            fclose(fp);
            return -1;
          }

          if(get_message(fp, conf->function) == -1)  //2
          {
            printf("no found the function choose!\n");
            fclose(fp);
            return -1;
          
          }
          if(get_message(fp,conf->path_http_white_list) == -1) //white.txt.http
          {
            printf("no found the path_http_white_list!\n");
            fclose(fp);
            return -1;
          }
          if(get_message(fp,conf->path_http_black_list) == -1)
          {
            printf("no found the path_http_black_list!\n");
            fclose(fp);
            return -1;
          }
