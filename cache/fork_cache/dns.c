...
//通过ｓｔｒｉｎｇ字符串首地址，获取需要解析的域名，并同时获取域名后面的类型和类。
...
void  dns_handler(u_char *user, const struct pcap_pkthdr* h, const u_char *bytes)
{
    ...
    int pay_size = 0;
    char server[NAME_LEN] ="192.168.90.22";
   ...
   int j = 0;
    ...
    if(dns->dns_flag != 0x0001)//dns如果不是标准请求0x0001，则退出
            return ;
...
//黑白名单源ＩＰ的匹配
   ...
   {
        printf("parse inquire name failed!\n");
        return ;
   }
   ...
   get_inquire.inquire_ip_len = strlen(name) + 2;//多加两个字符，是因为开头和结尾各有一个
...
//黑白名单的判断
   ...
   fprintf(fp_dns, "%s %s %02d %s %s %s\n","I",sys_time,1, s_ip_buf,name, server);
   //fflush(fp_dns);
        

        
            //ketama_select_server(cache_ip, path_server_ip, NULL, name);
            strcpy(cache_ip,server);
...
//build ipv4
   ...
   //memcpy(enet_src, eth->ether_dhost, 6);
    ...
    eth_tag = libnet_build_ethernet(
        ...
        if(eth_tag == -1)
        {
            printf("libnet_build_eth failure!\n");
            exit(1);
        }
