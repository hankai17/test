...
int file_compare_http_host(const  char *http, const int handle_method)
{
    printf("file check, http=%s\n",http);
    char *p = NULL;
    int  pmatch_eo = 0;
    int i = 0;
    int j = 0;
    if(handle_method == 0)
    {
        ...
        for(; i < now_count_http_white_list; i++)
        {
            ...
            printf("-------white regex check------------\n");
            printf("http=%s\n",http);
            printf("http_white_list[i].string=%s\n",http_white_list[i].string);
            printf("------------------------------------\n");
    ...
    if(handle_method == 1)
    {
        for(; i < now_count_http_black_list; i++)
        {
            ...
            {  
                printf("in black , over\n");
                return pmatch_eo;
            }   
        }
        printf("not in black , ok next\n");
        return -1;
    }
}
