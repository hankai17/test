...
int main(int argc, char *argv[]) {
    ...
    int i = 0;
    CAPTURE_POLL stDnsPoll; //将用以监听的fd与pcap 绑定在一起
    CAPTURE_POLL stHttpPoll;
    g_apstQueuControl[DNS_PCAP_HANDLE] = new_queue();  //DNS_PCAP_HANDLE=0 第一个队列
    g_apstQueuControl[HTTP_PCAP_HANDLE] = new_queue(); //HTTP_PCAP_HANDLE=1 第二个队列
    THREAD_PARAM stDnsThreadParam = { g_apstQueuControl[DNS_PCAP_HANDLE], //DNS参数结构体 : 队列句柄 捕捉回调句柄 类型
    ...
    signal(SIGTERM, sigroutine);  //注册一个命令
    if (function_choose == 1 || function_choose == 3) {
        sprintf(str_dns, "dst port 53");
        //  printf("the dev is %s\n",dev);
        if ((cp[DNS_PCAP_HANDLE] = pcap_filter_ready(dev, str_dns, errbuf))  //打开网卡 编译过滤规则 返回pcap句柄
                == NULL) {
            CMN_LOG(MY_LOG_DEBUG, "pcap_filter_ready failed!\n");
            exit(1);
        }
        selectable_dns_fd = pcap_get_selectable_fd(cp[DNS_PCAP_HANDLE]); //根据pcap句柄 获取一个可以用来监听的文件描述符
        ...
        pthread_create(&dns_poll_pid, NULL, capture_poll_thread,  //dns主线程 用于捕捉(捕捉必然有监听)
                (void *) &stDnsPoll);
        /*start dns process*/
        for (i = 0; i < dns_thread_num; ++i) {                    //dns子线程 用于解包(多个子线程肯定有竞争 所以有dns队列机制)
