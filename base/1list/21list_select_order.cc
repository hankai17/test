...
void* get_small_pre_node(list_node* head) {
  if(head == NULL) {
	return head;
  }
  list_node* small_pre = NULL; //最小点之前的值
  list_node* small = head; //最小值
  list_node* pre = head;
  list_node* cur = head->next;

  while(cur != NULL) {
	if(cur->data < small->data) { //记录两个值
	  small = cur;
	  small_pre = pre;
	}
	pre = cur;
...
void* select_sort(list_node* head) {
  if(head == NULL) {
	return head;
  }
  list_node* cur = head;
  list_node* small_pre = NULL;
  list_node* small = NULL;
  list_node* tail = NULL;
  while(cur != NULL) {
	small = cur;
	small_pre = (list_node *)get_small_pre_node(cur);
	if(small_pre != NULL) {
	...
	if(tail == NULL) {
	  ...
	  tail->next = small; //small 依次挂head后面
	}
	tail = small;
  }
  return head;
...
int main() {
  ...
  showlist_node(&head);
  list_node* tmp = ((list_node*)select_sort(&head)); 
  showlist_node(tmp);
  return 0;
}
