...
void tb_spider_ip::curl_http() 
{
    ...
    while(1)
    {
        std::chrono::milliseconds sleepDuration(333);
        ...
        if (curl != NULL)
        {
            ...
            if (CURLE_OK == unRet)
            {
                ...
                if (code >= 400)
                {
                    Log(__FILE__, __LINE__,Level[2],0,"%s~~%s|%s curlperform ok,but reponse 400",(char*)(ip_begin.data()),(char*)(ip_end.data()),(char*)(current_url.data()));
                }
            }
            else
            {
...
void tb_spider_ip::parse_data()
{
    ...
    std::cout<<current_ip<<" "<<s1<<"|"<<s2<<"|"<<s3<<"|"<<s4<<std::endl;
    std::string bar = s1 + "|" + s2 + "|" + s3 + "|" + s4;
    ...
    if(current_result == "" )
    {
        ...
        if(write_next_data != write_current_data)
        {
            ...
            if(pre_ip != current_ip)
            {
                ...
                std::string foo = pre_ip + "|" + current_ip_foo + "|" + bar + "\n"; 
