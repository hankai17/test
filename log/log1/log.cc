...
#define DEBUG_FILE	"spider_ip.log"
...
int  Level[5] = {IC_NO_LOG_LEVEL, IC_DEBUG_LEVEL, IC_INFO_LEVEL, IC_WARNING_LEVEL, IC_ERROR_LEVEL};
...
#ifdef WIN32
	sprintf(fileName, "c:\\process\\%s",DEBUG_FILE);
#else
	sprintf(fileName, "%s/%s", getenv("HOME"), DEBUG_FILE);
#endif
